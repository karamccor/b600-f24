...
A [random variable]{style="color: RosyBrown"} is a quantity whose value depends on the outcome of a random event. 
...
Ranodm variables encountered in this class will be either discrete or continuous. 

## Discrete random variables

[Discrete]{style="color: RosyBrown"} random variables are those that can take on a countable number of potential values (could be countably infinite!), each associated with a probability of occurring

This list of possible values and probabilities is the [probability distribution]{style="color: RosyBrown"}* for the discrete random variable in question.
...
This is known as a [Bernoulli]{style="color: RosyBrown"} random variable, and has a probability of "success" denoted by $p$
...
The [binomial distribution]{style="color: RosyBrown"}  gives the probability of $k$ "successes" from a sequence of $n$ independent Bernoulli trials. There are three assumptions:
...
The [Poisson distribution]{style="color: RosyBrown"} gives the probability that $k$ events occur in a given "interval". There are four assumptions:
...
Suppose we have a count random variable that follows a Poisson distributioni

- Since each event is independnt of others and the rate $\lambda$ is constant, **the probability that an event occurs within an interval is proportional to the length of that interval**. 
...
The [expected value]{style="color: RosyBrown"} of a discrete random variable $X$ is a weighted average of the possible outcomes:
...
The [variance]{style="color: RosyBrown"} of $X$ tells us how close values tend to be to its expectation:
